...
export const flowIdGuard = (flowId: any, res: Response, flowType: string): void => {
  if (!flowId && !isType<string>(flowId)) {
    if (flowType === 'logout') {
      res.redirect(`${process.env.KRATOS_BROWSER_URL}self-services/browser/flows/${flowType}`)
    }
    res.redirect(`${process.env.KRATOS_BROWSER_URL}self-services/${flowType}browser`)
  }
}
